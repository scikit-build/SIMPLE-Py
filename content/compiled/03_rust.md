...
The example module also exports `count_primes(limit)`, which counts the primes below `limit` by trial division &mdash; a deliberately branchy, CPU-bound loop, exactly the kind of code CPython executes slowly and a compiler loves.
...
Nineteen times faster, from a line-for-line transcription of the Python loop &mdash; no algorithm change, no tuning.
The exact digit varies by machine and by run; call it an order of 20×.

::: {warning} Benchmark release builds only

`maturin develop` compiles an *unoptimized* debug build by default &mdash; the fast-compile, slow-run profile meant for the edit&ndash;rebuild loop, the same trade-off as `cmake -DCMAKE_BUILD_TYPE=Debug`.
Depending on the workload, debug Rust can run 10&ndash;50× slower than release and can even lose to pure Python &mdash; which is where most "I rewrote it in Rust and it got slower!" stories come from.
...
* `#[new]` marks the constructor &mdash; `__init__`. Its body `Point { x, y }` builds the struct, using shorthand for `Point { x: x, y: y }`.
* `#[pyo3(get)]` generates a read-only attribute for a field &mdash; a `@property` without the boilerplate. There is no matching `set`, so assigning to `p.x` raises `AttributeError`.
...
`pyo3::exceptions` mirrors the builtins &mdash; `PyValueError`, `PyTypeError`, `PyKeyError`, and friends &mdash; and `new_err` constructs one with a message.
...
That is a genuine `ZeroDivisionError` &mdash; `except ZeroDivisionError:` catches it, and nothing about it betrays that the raiser was Rust.
...
One promise is still outstanding: the Rust source of `count_primes`, which hides the chapter's most Python-flavored topic &mdash; the Global Interpreter Lock.
...
The loop itself should read fine by now &mdash; `let mut` declares a variable that is allowed to change (Rust variables are immutable unless you say otherwise), and `2..limit` is `range(2, limit)`.
The interesting part is the frame around it:

* `py: Python<'_>` is a *token* proving this thread currently holds the GIL. PyO3 recognizes the parameter type and supplies it automatically &mdash; from Python, `count_primes` still takes a single argument.
* `py.detach(|| { ... })` runs the closure (`|| { ... }` is Rust's lambda) with the GIL released. Other Python threads get to run for the entire duration of the hot loop &mdash; the same courtesy NumPy extends around its C loops.
...
The GIL itself is on the way out: [free-threaded CPython](../interesting/free_threading.md) removes it entirely, and PyO3 is ahead of the curve &mdash; recent releases support free-threaded builds, where `py.detach` keeps its meaning (the thread detaches from the interpreter, so it no longer blocks stop-the-world operations like garbage collection).
...
That second tag is earned, not asserted: maturin bundles a reimplementation of `auditwheel` and audits every `maturin build` wheel automatically &mdash; work that is a separate tool and a separate CI step in the C++ chapters.
...
1. Add an `is_prime(n)` `#[pyfunction]` that reports whether a single number is prime &mdash; the trial-division test inside `count_primes` is the algorithm.
...
* `distance_to` goes inside the existing `#[pymethods] impl Point` block with the signature `fn distance_to(&self, other: &Point) -> f64` &mdash; PyO3 handles borrowing `other` when Python passes in a second `Point`. The body is two subtractions away from `magnitude`.
...
* The [PyO3 user guide](https://pyo3.rs/) &mdash; start with the getting-started walkthrough, then the chapters on classes and modules; everything this chapter glossed over is there.
* The [maturin user guide](https://www.maturin.rs/) &mdash; project layouts (including mixed Rust/Python packages), configuration, and the full CLI reference.
* [rust-numpy](https://github.com/PyO3/rust-numpy) &mdash; zero-copy NumPy arrays in Rust. The moment your hot data is an array, reach for this instead of converting element by element.
* [The Rust Programming Language](https://doc.rust-lang.org/book/) &mdash; "the Book", the canonical way to actually learn Rust; its first ten chapters cover every construct this example uses.
